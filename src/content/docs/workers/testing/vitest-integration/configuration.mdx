...
Use [`defineWorkersProject`](#defineworkersprojectoptions) with Vitest `test.projects` to specify a different configuration for certain tests. It should be used in place of the `defineProject()` function from Vitest.
